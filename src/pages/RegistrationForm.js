...
const RegistrationForm = () => {
    ...
    return (
        <div className="flex items-center h-screen flex-col relative">
            <div className='flex justify-between items-center mb-12 mt-4 w-full py-3 px-5'>
                <div className='ps-7'>
                    <Link to="/">
                        <svg width="121" height="32"
                            viewBox="0 0 121 32"
                ...
                <Link to="/"><RxCross2 className="text-[3rem] rounded-full border border-[#CACACA] p-2 font-semibold" /></Link>
            </div>
            {!submitted && (
                <div className="flex flex-col justify-center items-center w-[588px]">
                    <h1 className="font-secondary text-[36px] font-normal text-[#2DA950] text-center">Registration Form</h1>
                    <h1 className="font-semibold text-[56px] text-center font-primary text-[1c1c1c] leading-[3.5rem]">Start your success Journey here!</h1>
                    <div className="rounded-[64px] w-[417px] mt-16 mb-4 border">
                        <input
                            type="text"
                            placeholder="Enter your name"
                            value={name}
                            onChange={(e) => setName(e.target.value)}
                            className="py-[20px] px-[36px] rounded-[64px] w-full border-none bg-[#efefef] outline-[#537ff1] focus:bg-[#f5f8ff]"
                        />
                    </div>
                    <div className="rounded-[64px] w-[417px] mb-10">
                        <input
                            type="text"
                            placeholder="Enter your email"
                            value={email}
                            onChange={handleEmailChange}
                            className={`py-[20px] px-[36px] rounded-[64px] w-full border-none bg-[#efefef] outline-[#537ff1] focus:bg-[#f5f8ff] ${emailError && 'border-red-600'}`}
                        />
                        {emailError && <p className="text-red-600 text-sm w-full mt-3">{emailError}</p>}
                    </div>
                    <button
                        className={`rounded-[64px] border w-[417px] py-4  text-white ${email && name ? 'bg-[#1c1c1c]' : 'bg-[#c9c9c9]'} transition-colors`}
            ...
            {
                submitted && (
                    <div className="flex flex-col justify-center items-center w-[718px] px-24 mt-10">
                        <MdCheck className="text-[3.5rem] bg-[#2da950] rounded-full text-white p-2 my-6" />
                        <h1 className="font-secondary text-[36px] font-normal text-[#2DA950] text-center mb-4">Success Submitted</h1>
                        <h1 className="font-medium text-[56px] text-center font-primary text-[1c1c1c] leading-8 mb-8">Congratulations</h1>
                        <p className="font-primary font-medium text-[20px] text-[#727272] text-center">Your request has been successfully submitted to us. We will validate your information and reach out to your shortly with updates</p>
                        <span className="absolute bottom-4">Redirecting you to Homepage in <b>5 Seconds</b></span>
                    </div>
                )
            }
        </div>
